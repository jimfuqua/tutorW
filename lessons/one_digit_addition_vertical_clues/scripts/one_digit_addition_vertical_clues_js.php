...
function exit_from_lesson() {
    ...
    document.location.href = "/jimfuqua/tutor/src/scripts/cAssignment_get_next_lesson.php";
}
...
function record_answer(correct_answer) {
    var question_and_response = "vertical " + $('#UnitsDigitOne').text() + " + " + $('#UnitsDigitTwo').text() +
        " = " + answer_entered;
  // use ajax to record answer
	...
	post_string = {
		...
		"lesson_id" : lesson_id,
        "sender" : "one_digit_addition_vertical_clues_js",
        "tA_RepsTowardM" : tA_RepsTowardM
		};

    update_tCompleted(post_string);
    $.post("http://localhost/jimfuqua/tutor/src/scripts/update_tAssignments.php", post_string);
}
