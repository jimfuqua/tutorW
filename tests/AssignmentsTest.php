...
require_once '../src/classes/SplitsClass.inc';

class AssignmentsClass extends PHPUnit_Framework_TestCase
{
    ...
    public function testbuildArray()
    {
        ...
        $this->assertTrue(count($this->myArray) === 23, "Count of myArray should be 23.");
        return ($this->myArray);
    }//end testbuildArray()


    /**
     * Test test "getAssignmentsByStudentID()" in Assignments.class.inc
     *
     * The method "getAssignmentsByStudentID()" does what it says.
     *
     * @return void
     */
    ...
    /**
     * Test test "testgetOneRowFromDbAsArrayID()" in Assignments.class.inc
     *
    ...
    /**
     * Test test "getSpecificStudentAssignmentFromDbAsArray()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    public function testgetSpecificStudentAssignmentFromDbAsArray()
    {
        $this->testbuildArray();
        // reset myArray to origninal values.
        $classInstance = new classes\AssignmentsClass;
        $result        = $classInstance->delRowsByStudentId($this->myArray['tA_S_ID']);
        $this->assertTrue($result === 0);
        $this->myArray['tA_S_ID']           = 'ghghgh';
    ...
    /**
     * Test "compareClassMethodsToTests()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test test "deleteRowByRowId()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test test "getNewestDbEntry()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test "getSumOfAssignedTimeFromArray()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test "insertRecord()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
...
$valueArray = array(
               'tA_ErrorsMade' => '2',
               'tA_StartRec'   => '25',
              ); = fopen("/var/www/jimfuqua/tutor/logs/testinsertRecord.log", "w");
    ...
    /**
     * Test test "fieldsInDbVsMyArray()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    public function testfieldsInDbVsMyArray()
    {
        ...
        $result        = $classInstance->insertRecord($this->myArray);
        $row           = $classInstance->getLastDbEntryAsArray($this->myArray['tA_S_ID']);
        // Row has both numeric and relational indexing which doubles it's number.
        $this->assertTrue(count($row) === count($this->myArray));

        // $row has both an associative and numeric array.  Must strip numeric.
        $i = 0;
//         // strip numberic keys.
...
//         }
//
//             // compare $row to $this->myArray key by key and value by value.
...
//         }
//
//             $count_row = count($myArray_key_array);
...
//             $classInstance->delRowsByStudentId($this->myArray['tA_S_ID']);
    ...
    /**
     * Test test "getLastDbEntryAsArray()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test test "getOneRowFromDB()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test test "deleteLastRow()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test test "getCurrentStudentAssignmentsInAnArray()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
...
function testChange_tA_Post_date(){//merge with Assignmentstest
    ...
    /**
     * Test test "updateFields()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test test "updateTaRepsTowardM()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
     * @return void
     */
    public function testupdateTaRepsTowardM()
    {
        ...
        $this->assertTrue($result === 1);  // Number or rows affected.
        $row = $classInstance->getLastDbEntryAsArray($this->myArray['tA_S_ID']);
        $returnArray = $classInstance->updateTaRepsTowardM(
            ($this->myArray['tA_RepsTowardM'] + 3) ,
            $this->myArray['tG_Reps_to_master'],
            $row['tA_id']);
        $newValue = $this->myArray['tA_RepsTowardM'] + 3;  // a number before conversion
        $newValue =  "'".$newValue."'";
        $newValue =  trim($newValue);
        $returnArray["tA_RepsTowardM"] = "'".$returnArray["tA_RepsTowardM"]."'";
        $this->assertTrue($returnArray['tA_RepsTowardM'] ===  $newValue);
        // Clean up.
        $rowsDeleted = $classInstance->deleteRowByRowId($row['tA_id']);
        $this->assertTrue($rowsDeleted ===  1);

    }//end testupdateTaRepsTowardM()


    /**
     * Test test "updateTaStartRec()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test test "getAssignmentByAssignmentID()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test test "okToRepeatAssignment()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
     * @return void
     */
    public function testokToRepeatAssignment()
    {
        // load a lesson into db
        $this->testbuildArray();
        // reset myArray to origninal values.
        $tempStudent = 'abcdefg';
        $this->myArray['tA_S_ID'] = $tempStudent;
        ...
        if ($result == 1) {
            ...
            $array_result = $classInstance->getSpecificStudentAssignmentFromDbAsArray(
                $tempStudent,
                $this->myArray['tG_AssignmentName'],
                $this->myArray['tA_StartRec']
            );
            // $resultArray = array();
            // while ($row = mysql_fetch_assoc($retrieved_result)) {
            // array_push($resultArray, $row);
            // }
            // check the fields
            $this->assertTrue($array_result['tA_S_ID'] === $tempStudent);
            // $thisTest = $classInstance -> okToRepeatAssignment($tempStudent,
            // $this->myArray['tG_AssignmentName'], $this->myArray['tA_StartRec']);
            // $this -> assertTrue($thisTest === '0');
            // }
            // remove lesson from array but not db if set
            unset($classInstance);
            $classInstance = new tutor\src\classes\AssignmentsClass;
            $classInstance->delRowsByStudentId($tempStudent);
            // insure no previous lessons for this student.
            $this->myArray['tA_Consecutive_Reps_OK'] = TRUE;
            $result = $classInstance->insertRecord($this->myArray);
            // Returns 1 or FALSE
            if ($result === 1) {
                // The test record was added to the DB.
                $retrieved_row = $classInstance->getSpecificStudentAssignmentFromDbAsArray(
                    $tempStudent,
                    $this->myArray['tG_AssignmentName'],
                    $this->myArray['tA_StartRec']
                );
                $this->assertTrue($retrieved_row['tA_S_ID'] == $tempStudent);
                $thisTest = $classInstance->okToRepeatAssignment(
                    $tempStudent,
                    $this->myArray['tG_AssignmentName'],
                    $this->myArray['tA_StartRec']
                );
                $this->assertTrue($thisTest === '1');
            }
    ...
    /**
     * Test test "printNullIfNull()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test test "repairRowIfStartRecIslargerThanStopRec()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    public function testrepairRowIfStartRecIslargerThanStopRec()
    {
        ...
        // Next line needs a specfic test.
        $this->assertTrue($result['tA_ErroneousAssignmentDeleted'] === FALSE);
        $log_file
               'tA_S_ID'       => 'bcdefgh',
               'tA_ErrorsMade' => '4',
              );
$valueArray = array(
               'tA_ErrorsMade' => '2',
               'tA_StartRec'   => '25',
              ); = fopen("/var/www/jimfuqua/tutor/logs/testrepairRowIfStartRecIslargerThanStopRec.log", "w");
$v = var_export($result, true);
$string = __LINE__.' test $result = '.$v."\n";
fwrite($log_file
               'tA_S_ID'       => 'bcdefgh',
               'tA_ErrorsMade' => '4',
              );
$valueArray = array(
    ...
    /**
     * Test "testgetSumOfAssignedTime()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
    ...
    /**
     * Test "normalizePercentTimeTo100Percent()" in Assignments.class.inc
     *
     * A *description*, that can span multiple lines, to go _in-depth_ into the details of this element
     * and to provide some background information or textual references.
     *
...
//        $this->assertTrue($result === 1);
    ...
    public function testcheckAndProcessSplits()
    {
        ...
        $this->assertTrue(count($myArray2) === 4);
        $splitInstance = new classes\SplitsClass;
        // Next insert into db.
        $splitInstance->deleteRowId($myArray1['tSp_id']);
        $splitInstance->insertRow($myArray1);
        $splitInstance->deleteRowId($myArray2['tSp_id']);
...
$valueArray = array(
               'tA_ErrorsMade' => '2',
               'tA_StartRec'   => '25',
              ); = fopen("/var/www/jimfuqua/tutor/logs/testgetNextAssignmentToDo.log", "w");
// $v = var_export($result, true);
